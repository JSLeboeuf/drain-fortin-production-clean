...
import { Wrench, BarChart3, Phone, Settings, FlaskConical, Gauge, Activity } from "lucide-react";
...
export default function AccessibleSidebar({ className, onNavigate }: AccessibleSidebarProps) {
  const [location] = useLocation();
  const skipLinkRef = useRef<HTMLAnchorElement>(null);
  const mainContentRef = useRef<HTMLElement | null>(null);

  // Set up main content reference for skip link
  useEffect(() => {
    mainContentRef.current = document.querySelector('[role="main"]') || 
      ...
      >
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p 
                ...
                data-testid="text-logo-subtitle"
                ...
                <li key={item.name} role="listitem">
                  <Link href={item.href}>
                    ...
                    >
                      ...
                      <span 
                        id={`nav-desc-${item.name.toLowerCase().replace(/\s+/g, '-')}`}
                        className="sr-only"
                      >
                        {item.description}
                      </span>
        ...
        >
          <div className="flex items-center space-x-2 text-sm">
            <div 
              className="w-2 h-2 bg-drain-green-500 rounded-full animate-pulse"
              role="img"
              aria-label="Indicateur de statut: système opérationnel"
            ></div>
            <span 
